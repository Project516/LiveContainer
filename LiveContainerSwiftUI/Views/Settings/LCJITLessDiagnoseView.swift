    ...
    func onAppear() {
        ...
        if let entitlementTeamId {
            ...
            if let keyChainAccessGroups = entitlementDict["keychain-access-groups"] as? Array<String> {
                var notFound = true
                if keyChainAccessGroups.contains("\(entitlementTeamId).org.spectrum3847.spectrum-mobile.shared") {
                    notFound = false
                    for i in 1..<SharedModel.keychainAccessGroupCount {
                        if !keyChainAccessGroups.contains("\(entitlementTeamId).org.spectrum3847.spectrum-mobile.shared.\(i)") {
                            notFound = true
                            continue
                        }
                    }
